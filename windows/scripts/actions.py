...
def change_light_setting_new(conditions, context, network, parameters):
    """
    ...
    greenTime = [0]*2
    greenTime[0] = conditions.values()[0]["greenTime1"]      # two green time
    greenTime[1] = conditions.values()[0]["greenTime2"]

    seqNum = 0
    for seq in pdf.GetLstSequences():
		duree_seq = greenTime[seqNum]
		seq.SetTotalLength(duree_seq)
		newSeq = symuvia.CreateSequence(duree_seq, seqNum)
		#maj des signaux
		
		for signal in seq.GetLstActiveSignals():
			signal.GreenDuration = duree_seq
...
def change_distribution(conditions, context, network, parameters):
	"""
	Change la répartion des véhicules sur les voies à une entrée
	"""
	c = str()
	
	coeff = symuvia.vectorDouble()
	scoeff = parameters["coeffs"]
	sTmp = scoeff.split()
	coeff.extend(map(float,sTmp))
	
	entree = network.GetOrigineFromID(parameters["entree"],c)
	typevehicule = network.GetVehicleTypeFromID(parameters["typevehicule"])
	
	entree.SetLstRepVoie(typevehicule,coeff)
